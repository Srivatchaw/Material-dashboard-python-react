// src/layouts/items/edit/index.js
...
function EditItem() {
  ...
  // const [description, setDescription] = useState(""); // <--- REMOVED from useState
  const [startDate, setStartDate] = useState("");
  const [expectedCompletionDate, setExpectedCompletionDate] = useState("");
  const [actualCompletionDate, setActualCompletionDate] = useState("");
  const [status, setStatus] = useState("Pending");
  // const [reasonForDelay, setReasonForDelay] = useState(""); // <--- REMOVED from useState
  ...
  const fetchItemDetails = async () => {
    if (!user || !user.user_id || !item_id) {
      openSnackbar("Authentication or Item ID missing.", "error");
      setLoading(false);
      navigate("/authentication/sign-in");
      return;
    }
    try {
      const response = await axios.get(`http://127.0.0.1:5000/api/items/get/${item_id}`, {
        headers: {
      ...
      setProjectName(itemData.project_name || "");
      setFormName(itemData.form_name || "");
      // setDescription(itemData.description || ""); // <--- REMOVED
      setStartDate(itemData.start_date || "");
      setExpectedCompletionDate(itemData.expected_completion_date || "");
      setActualCompletionDate(itemData.actual_completion_date || "");
      setStatus(itemData.status || "Pending");
      // setReasonForDelay(itemData.reason_for_delay || ""); // <--- REMOVED
      // Pre-fill NEW fields
      setCustomer(itemData.customer || "");
      setPublicIp(itemData.public_ip || "");
      setPrivateIp(itemData.private_ip || "");
      setOsType(itemData.os_type || "");
      setRootUsername(itemData.root_username || "");
      setRootPassword(itemData.root_password || "");
      setServerUsername(itemData.server_username || "");
      setServerPassword(itemData.server_password || "");
      setServerName(itemData.server_name || "");
      setCore(itemData.core ? String(itemData.core) : "");
      setRam(itemData.ram || "");
      setHdd(itemData.hdd || "");
      setPorts(itemData.ports || "");
      setLocation(itemData.location || "");
      setApplications(itemData.applications || "");
      setDbName(itemData.db_name || "");
      setDbPassword(itemData.db_password || "");
      setDbPort(itemData.db_port ? String(itemData.db_port) : "");
      setDumpLocation(itemData.dump_location || "");
      setCrontabConfig(itemData.crontab_config || "");
      setBackupLocation(itemData.backup_location || "");
      setUrl(itemData.url || "");
      ...
      if (error.response) {
        ...
        openSnackbar("Error: No response from server when fetching item details.", "error");
      } else {
        openSnackbar("An unexpected error occurred while fetching item details.", "error");
      }
  ...
  const handleUpdateItem = async (event) => {
    ...
    if (!user || !user.user_id || !item_id) {
      openSnackbar("Authentication or Item ID missing for update.", "error");
      navigate("/authentication/sign-in");
      return;
    }
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:5000/api/items/update/${item_id}`,
        {
      ...
      if (error.response) {
        openSnackbar(`Error updating item: ${error.response.data.message}`, "error");
      } else if (error.request) {
        openSnackbar("Error: No response from server. Check if the backend is running.", "error");
      } else {
        openSnackbar("An unexpected error occurred during item update.", "error");
      }
...
// PropTypes (if EditItem receives any props)
EditItem.propTypes = {
  // Add propTypes here if EditItem is expected to receive props (e.g., initial item ID)
  // item_id: PropTypes.string.isRequired, // if passed directly, but we use useParams()
};
